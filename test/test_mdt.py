...
class TableTests(MDTTest):
    ...
    def test_bad_read(self):
        ...
        mlib = self.get_mdt_library()
        _ = mdt.features.ResidueType(mlib)
        feat = """
  #  FEATURE NBINS NAME
...
"""
    ...
    def test_guess_chunk_size(self):
        ...
        self.assertEqual(m._guess_chunk_size([12, 6, 6, 12, 306, 306],
                                             1024 * 1024 * 10),
                         [6, 3, 3, 6, 161, 161])
    ...
    def test_residue_span_range(self):
        ...
        tuple_dist = mdt.features.TupleDistance(
            mlib, bins=mdt.uniform_bins(49, 2.0, 0.2))
    ...
    def test_feature_combination(self):
        ...
        atmdist = mdt.features.AtomDistance(mlib,
                                            bins=mdt.uniform_bins(60, 0, 0.5))
        resdist = mdt.features.ResidueDistance(
            mlib, protein=1, bins=mdt.uniform_bins(7, 0, 2.0))
    ...
    def test_save_reshape(self):
        ...
        restyp1 = mdt.features.ResidueType(mlib, protein=1)
        m = self.get_test_mdt(mlib, features=(restyp0, restyp1))
        m = m.reshape(features=(restyp0, restyp1), offset=(4, 2),
                      shape=(11, 10))
        m.write('test.mdt')
        m.write_hdf5('test.hdf5')
    ...
    def test_reshape(self):
        ...
        m3 = m.reshape(features=(restyp0, restyp1), offset=(0, 0),
                       shape=m.shape)
        self.assertMDTsEqual(m, m3)
        m3 = m.reshape(features=(restyp0, restyp1), offset=(4, 2),
                       shape=(11, 10))
        # Reshaping to same offset and shape should be a no-op
        m4 = m3.reshape(features=(restyp0, restyp1), offset=(4, 2),
                        shape=(11, 10))
    ...
    def test_normalize(self):
        ...
        m2 = m.normalize(dimensions=2, dx_dy=(1, 1), to_zero=False,
                         to_pdf=True)
        inds = []
        while self.roll_inds(inds, m.shape, m.offset):
            self.assertAlmostEqual(1. / 484., m2[inds], places=3)
        ...
        m2 = m.normalize(dimensions=2, dx_dy=(1, 1), to_zero=True,
                         to_pdf=False)
    ...
    def test_pass_cutoffs(self):
        ...
        dummy_bin = DummyBin()
        self.assertEqual(mdt._pass_cutoffs(
            dummy_table, 0, dummy_bin, density_cutoff=None,
            entropy_cutoff=None), True)
        self.assertEqual(mdt._pass_cutoffs(
            dummy_table, 0, dummy_bin, density_cutoff=10.1,
            entropy_cutoff=None), False)
        self.assertEqual(mdt._pass_cutoffs(
            dummy_table, 0, dummy_bin, density_cutoff=None,
            entropy_cutoff=19.9), False)
        self.assertEqual(mdt._pass_cutoffs(
            dummy_table, 0, dummy_bin, density_cutoff=9.9,
            entropy_cutoff=20.1), True)
